...
class dataset_single(data.Dataset):
    def __init__(self, opts, setname, input_dim):
        ...
        self.img = [os.path.join(
            self.dataroot, opts.phase + setname, x) for x in images]
...
class dataset_unpair(data.Dataset):
    def __init__(self, opts):
        ...
        self.A = [os.path.join(self.dataroot, opts.phase + 'A', x)
                  for x in images_A]

        # B
        images_B = os.listdir(os.path.join(self.dataroot, opts.phase + 'B'))
        self.B = [os.path.join(self.dataroot, opts.phase + 'B', x)
                  for x in images_B]
    ...
    def __getitem__(self, index):
        if self.dataset_size == self.A_size:
            ...
            data_B = self.load_img(
                self.B[random.randint(0, self.B_size - 1)], self.input_dim_B)
        else:
            data_A = self.load_img(
                self.A[random.randint(0, self.A_size - 1)], self.input_dim_A)
